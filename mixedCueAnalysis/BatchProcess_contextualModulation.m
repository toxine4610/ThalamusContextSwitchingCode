...
% function BatchProcess_contextualModulation
...
datapath = '../ForContextSwitchProject/DoubleCueDatabase/Production PFCSuppression Unilat';
...
for m = 1:length(MouseID)
    ...
    for d = 2:length(dates_this_mouse)
        ct = ct+1;
        fprintf('  Processing date: %s\n', dates_this_mouse{d});
        ST = load( [datapath filesep mouse filesep dates_this_mouse{d} filesep MyMouse{m} '_pfchalo_session' ] );
        [Spfc_mix, Smd_mix, ZVis, ZAud] = packageData(ST.Z_C1, ST.Smd, ST.Spfc);
        [ExpH{m}{d}, perf1{m}{d}, Toss{m}{d}] = getBehavior(ST.Z_C1);
        pCorrect(ct) = length( find(Toss{m}{d} == 1) )./length( Toss{m}{d} );
...
for i = 1:numel(SpfcMega)
    ...
    % correct =============================================================
    R1C1  = SpfcMega(i).SpikeTimes_R1_sound;
    R2C1  = SpfcMega(i).SpikeTimes_R2_sound;
    
    R1C2  = SpfcMega(i).SpikeTimes_R1_nosound;
    R2C2  = SpfcMega(i).SpikeTimes_R2_nosound;
    ...
    R1C1  = SpfcMega(i).SpikeTimes_R1_sound_inc;
    R2C1  = SpfcMega(i).SpikeTimes_R2_sound_inc;
    
    R1C2  = SpfcMega(i).SpikeTimes_R1_nosound_inc;
    R2C2  = SpfcMega(i).SpikeTimes_R2_nosound_inc;
...
for i = 1:numel(SmdMega)
    ...
    % correct =============================================================
    R1C1  = SmdMega(i).SpikeTimes_R1_sound;
    R2C1  = SmdMega(i).SpikeTimes_R2_sound;
    
    R1C2  = SmdMega(i).SpikeTimes_R1_nosound;
    R2C2  = SmdMega(i).SpikeTimes_R2_nosound;
    ...
    R1C1  = SmdMega(i).SpikeTimes_R1_sound_inc;
    R2C1  = SmdMega(i).SpikeTimes_R2_sound_inc;
    
    R1C2  = SmdMega(i).SpikeTimes_R1_nosound_inc;
    R2C2  = SmdMega(i).SpikeTimes_R2_nosound_inc;
...
% plot(  abs(MD_muFR_BAS_C1), abs(MD_muFR_BAS_C1) ,'o','markersize',10,'markeredgecolor','w','markerfacecolor','g' ); hold on;
plot(  abs(MD_CMI_CFD), abs(MD_CMI_CFD_bl) ,'o','markersize',10,'markeredgecolor','w','markerfacecolor','g' ); hold on;
