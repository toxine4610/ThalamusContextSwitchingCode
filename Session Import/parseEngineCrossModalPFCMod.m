function [ initT, trial, laser, LEDTBAud, right_trials, left_trials, stim_lengths, catchh, audioStim, visStim, audioStimDistract, visStimDistract, corr, inc, ts, cueL, cueR, ITI, LEDstate] = parseEngineCrossModalPFCMod( filename, MouseID)
...
for ii = 1:length(LEDstate)
    ...
    switch state
        case 'init'
            if strfind(transition, 'laser on')
